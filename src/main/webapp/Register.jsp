<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" session="false" %>
	<%@ taglib prefix="c" uri="jakarta.tags.core" %>
		...
		<div class="w3-card card w3-section">
			<form id="registerForm" class="auth-form" action="Register" method="POST" enctype="multipart/form-data">
				...
				<div class="field-group">
					...
					<input class="w3-input" type="text" id="surname" name="surname" required minlength="1"
						maxlength="20" value="${user.surname}" title="Surname must be between 1 and 20 characters." />
				</div>

				<div class="field-group">
					<label for="nickname">Nickname</label>
					<input class="w3-input" type="text" id="nickname" name="nickname" required minlength="1"
						maxlength="20" value="${user.nickname}" title="Nickname cannot be empty." />
				</div>

				<div class="field-group">
					<label for="email">Email</label>
					<input class="w3-input" type="email" id="email" name="email" required value="${user.email}"
						title="Must be a valid email." />
				</div>

				<div class="field-group">
					<label for="initialPassword">Password</label>
					<input class="w3-input" type="password" id="initialPassword" name="password" required
				...
				<div class="field-group">
					...
					<input class="w3-input" type="text" id="favoriteGame" name="favoriteGame"
		...
		<script>
			App.Errors = {
				<c:forEach var="error" items="${errors}">"${error.key}": "${error.value}",</c:forEach>
			};
